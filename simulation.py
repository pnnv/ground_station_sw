...
INTERVAL = 0.05  # seconds between entries (50ms)
...
wind_speed = random.uniform(-2, 2)  # m/s horizontal wind
wind_gust_chance = 0.02
sensor_glitch_chance = 0.01
...
while True:
    ...
    if random.random() < wind_gust_chance:
        ...
        wind_speed = max(-5, min(5, wind_speed))

    # Update dynamics
    if phase == "ASCENT":
        acc = acceleration + random.uniform(-0.5, 0.5)  # add some noise
        vertical_speed += acc * INTERVAL
    elif phase == "DESCENT":
        acc = descent_acceleration + random.uniform(-0.2, 0.2)
        vertical_speed += acc * INTERVAL
    elif phase == "COAST":
        vertical_speed *= 0.99 + random.uniform(-0.005, 0.005)  # simulate drag and wind

    altitude += vertical_speed * INTERVAL
    altitude = max(0.0, altitude)
    gps_alt = altitude

    # GPS drift and wind effect
    gps_lat += random.uniform(-0.00002, 0.00002) + wind_speed * 0.0000005
    gps_lon += random.uniform(-0.00002, 0.00002) + wind_speed * 0.0000005

    # Battery voltage drop with random spikes
    voltage = max(9.0, voltage - voltage_drop_per_entry + random.uniform(-0.0005, 0.0005))

    # Sensor readings with noise and occasional glitches
    if random.random() < sensor_glitch_chance:
        a_x = random.uniform(-2, 2)
        a_y = random.uniform(-2, 2)
        a_z = random.uniform(-2, 2)
        gyro_x = random.uniform(-50, 50)
        gyro_y = random.uniform(-50, 50)
        gyro_z = random.uniform(-50, 50)
    else:
        a_x = random.uniform(-0.3, 0.3)
        a_y = random.uniform(-0.3, 0.3)
        a_z = (vertical_speed - (vertical_speed - acc * INTERVAL)) / INTERVAL + random.uniform(-0.1, 0.1)
        gyro_x = random.uniform(-8, 8)
        gyro_y = random.uniform(-8, 8)
        gyro_z = random.uniform(-8, 8)

    angle_x = a_x * 5 + random.uniform(-2, 2)
    angle_y = a_y * 5 + random.uniform(-2, 2)
    angle_z = gyro_z * 0.1 + random.uniform(-1, 1)
